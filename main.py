...
def collect_audit_for_environment(environment, source_ip, source_shared_directory, source_path, save_ip, save_shared_directory, save_path):
	...
	with smb_connector(source_ip, source_shared_directory, 'vmadmin', 'Password100') as source_connection:
...
if __name__ == "__main__":
	try:
		...
		save_ip = sys.argv[5]
		save_shared_directory = sys.argv[6]
		save_path = sys.argv[7]
	except:
		print('Bad parameters passed')
		sys.exit()
	else:
		collect_audit_for_environment(environment, source_ip, source_shared_directory, source_path, save_ip, save_shared_directory, save_path)